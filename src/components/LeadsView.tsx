...
export default function LeadsView() {
  ...
  // Advanced Export CSV States
  const [isExportModalOpen, setIsExportModalOpen] = useState(false);
  const [exportScope, setExportScope] = useState<'all' | 'filtered'>('filtered');
  const [selectedExportColumns, setSelectedExportColumns] = useState<string[]>([
    "Name", "Email", "Phone", "Source", "Status", "Lead Score", "Tags", "Expected Value", "Target Course", "Target Band", "Destination", "Created At"
  ]);
  ...
  const handleExportCSV = () => {
    ...
    setExportScope(sortedLeads.length > 0 ? 'filtered' : 'all');
    setIsExportModalOpen(true);
  };

  const executeCSVExport = () => {
    const activeLeads = exportScope === 'all' ? leads : sortedLeads;
    if (activeLeads.length === 0) {
      alert("No leads found for the selected export scope.");
      return;
    }

    const columnGetters: Record<string, (lead: Lead) => any> = {
      "Name": (lead) => lead.name,
      "Email": (lead) => lead.email,
      "Phone": (lead) => lead.phone,
      "Source": (lead) => lead.source,
      "Status": (lead) => lead.status,
      "Lead Score": (lead) => calculateLeadScore(lead).score,
      "Tags": (lead) => lead.tags ? lead.tags.join("; ") : "",
      "Expected Value": (lead) => lead.expectedValue || "",
      "Target Course": (lead) => lead.targetCourse || "",
      "Target Band": (lead) => lead.targetBand || "",
      "Destination": (lead) => lead.destination || "",
      "Created At": (lead) => lead.createdAt ? format(new Date(lead.createdAt), "yyyy-MM-dd HH:mm:ss") : "",
    };

    const activeHeaders = selectedExportColumns.filter(col => col in columnGetters);
    ...
    if (includeHeaders) {
      csvRows.push(activeHeaders.map(h => `"${h.replace(/"/g, '""')}"`).join(csvDelimiter));
    }
    rows.forEach(row => {
      csvRows.push(
        row.map(val => `"${String(val ?? "").replace(/"/g, '""')}"`).join(csvDelimiter)
      );
    });
    ...
    const baseName = customFileName.trim() 
      ...
      : `crm_leads_${exportScope}_export`;
    const formattedDate = format(new Date(), "yyyyMMdd_HHmmss");
    
    link.setAttribute("download", `${baseName}_${formattedDate}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    logAuditEvent({
      ...
      entityType: "lead",
      entityId: "bulk",
      details: `Exported ${activeLeads.length} leads under "${exportScope}" scope. Columns included: ${activeHeaders.join(", ")}.`
    });
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Advanced Export CSV Configurator Modal */}
      {isExportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-200">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-base font-semibold text-slate-900 font-display">
                    Export CRM Data Report
                  </h2>
                  <p className="text-xs text-slate-500 font-medium">Build a tailored spreadsheet of lead metrics</p>
                </div>
              </div>
              <button
                onClick={() => setIsExportModalOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <span className="text-xs font-semibold text-slate-800">Filtered View</span>
                    <span className="text-[10px] text-slate-550 mt-1">
                      Export only current search/filters ({sortedLeads.length} leads matched)
                    </span>
                  </button>
                  <button
                  ...
                  >
                    <span className="text-xs font-semibold text-slate-800">Entire Database</span>
                    <span className="text-[10px] text-slate-550 mt-1">
                      Download complete CRM archives ({leads.length} leads in total)
                    </span>
                  </button>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-[10px] text-indigo-600 hover:text-indigo-805 font-bold hover:underline"
                    >
                      All Columns
                    </button>
                    <span className="text-slate-300 text-xs">|</span>
                    <button
                      onClick={() => setSelectedExportColumns(["Name", "Email", "Phone"])}
                      className="text-[10px] text-indigo-600 hover:text-indigo-805 font-bold hover:underline"
                    >
                      Contacts Only
                    </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 bg-slate-50 border border-slate-200/60 rounded-xl p-3">
                  {[
                    "Name", "Email", "Phone", "Source", "Status", "Lead Score", "Tags", "Expected Value", "Target Course", "Target Band", "Destination", "Created At"
                  ].map((col) => {
                    const isChecked = selectedExportColumns.includes(col);
                    return (
                      <label
                        key={col}
                        className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-white border border-transparent hover:border-slate-100 transition-all text-xs font-medium text-slate-700 cursor-pointer text-ellipsis overflow-hidden"
                      >
                        <input
                          type="checkbox"
                          checked={isChecked}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full text-xs font-medium border border-slate-200 rounded-xl px-3 py-2 bg-white"
                  >
                ...
                <div>
                  ...
                  <label className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 py-2 bg-white cursor-pointer h-9">
                    <input
                      type="checkbox"
                      checked={includeHeaders}
                      onChange={(e) => setIncludeHeaders(e.target.checked)}
                      className="rounded text-indigo-600 focus:ring-indigo-505 size-4"
                    />
                    <span className="text-xs font-semibold text-slate-700 select-none">Include Title Row</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="crm_leads_export"
                    className="w-full text-xs font-medium border border-slate-200 rounded-xl pl-3 pr-16 py-2.5 bg-white text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-indigo-505 focus:border-indigo-505"
                  />
                  <span className="absolute right-3.5 top-2.5 text-[10px] text-slate-400 font-bold select-none uppercase tracking-wide">
                    .CSV
                  </span>
                </div>
              </div>
            ...
            {/* Footer */}
            <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex-shrink-0 flex justify-end gap-2.5">
              <button
                onClick={() => setIsExportModalOpen(false)}
                className="px-4 py-2 text-xs font-semibold text-slate-550 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={executeCSVExport}
                className="px-5 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm transition-all cursor-pointer flex items-center gap-2"
              >
                <Download className="w-3.5 h-3.5" />
                <span>
                  Export {exportScope === 'all' ? leads.length : sortedLeads.length} Records
                </span>
              </button>
            </div>
          </div>
